...
![](../assets/985bfa1f7e6b5200ef0e0b30ddf63fc9_2.png)

chrome:

![](../assets/985bfa1f7e6b5200ef0e0b30ddf63fc9_3.png)
...
![](../assets/985bfa1f7e6b5200ef0e0b30ddf63fc9_4.png)
...
![](../assets/985bfa1f7e6b5200ef0e0b30ddf63fc9_5.png)

可以看到，当登录用户访问攻击者的网站时，会向 www.c.com 发起一个删除用户帖子的请求。此时若用户在切换到 www.c.com 的帖子页面刷新，会发现ID 为 87343 的帖子已经被删除。

由于浏览器中 HTTP(s) 请求是会自动帮我们把 Cookie 带上传给服务端。当用户访问攻击者准备的攻击环境时，攻击者就可以访问用户登录过的网站进行CSRF攻击。在这个攻击过程中，攻击者借助受害者的 Cookie 骗取服务器的信任，但并不能拿到 Cookie，也看不到 Cookie 的内容。

而对于服务器返回的结果，由于浏览器同源策略的限制，攻击者也无法进行解析。因此，攻击者无法从返回的结果中得到任何东西，他所能做的就是给服务器发送请求，以执行请求中所描述的命令，在服务器端直接改变数据的值，而非窃取服务器中的数据。
...
![](../assets/985bfa1f7e6b5200ef0e0b30ddf63fc9_6.png)
